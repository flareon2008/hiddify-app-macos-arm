...
Future<void> lazyBootstrap(WidgetsBinding widgetsBinding, Environment env) async {
  ...
  if (PlatformUtils.isDesktop) {
    ...
    final hideFromDock = container.read(Preferences.hideFromDock);
    Logger.bootstrap.debug("silent start [${silentStart ? "Enabled" : "Disabled"}]");
    Logger.bootstrap.debug("hide from dock [${hideFromDock ? "Enabled" : "Disabled"}]");
    if (!silentStart && !hideFromDock) {
      await container.read(windowNotifierProvider.notifier).show(focus: false);
    } else {
      Logger.bootstrap.debug("silent start or hide from dock, remain hidden accessible via tray");
    }
